...
CONFIG=$(tar xf ./tmp/step5.tar manifest.json -O | jq -r '.[0].Config')
tar xf ./tmp/step5.tar "$CONFIG" -O | jq '.config.Env'
...
docker save oci-demo:step5-builder -o ./tmp/step5-builder.tar

CONFIG=$(tar xf ./tmp/step5-builder.tar manifest.json -O | jq -r '.[0].Config')
tar xf ./tmp/step5-builder.tar "$CONFIG" -O | jq '.config.Env'
